...
public class VinylBookController implements Initializable
{
    ...
    public void initService(VinylsService service)
    {
        this.service = service;

        service.onGetAllVinyls(response ->
        {
            Platform.runLater(() ->
            {
                viewModel.setVinyls(response.getVinyls());
                renderVinyls();
            });
        });

        service.onBorrowVinyl(response ->
        {
            Platform.runLater(() ->
            {
                Vinyl vinyl = response.getVinyl();
                viewModel.updateVinyl(vinyl);
                renderVinyls();
            });
        });

        service.onReserveVinyl(response ->
        {
            Platform.runLater(() ->
            {
                Vinyl vinyl = response.getVinyl();
        ...
        service.onReturnVinyl(response ->
        {
            Platform.runLater(() ->
            {
                Vinyl vinyl = response.getVinyl();
        ...
        service.onVinylUpdated(message ->
        {
            Platform.runLater(() ->
            {
    ...
    private void loadVinyls()
    {
        ...
        service.getAllVinylsFromList(new GetAllVinylsRequest());
    }
        ...
        ImageView imageView = new ImageView(
            new Image(
                Objects.requireNonNull(
                    getClass().getResourceAsStream("/Images/pic.png")
                )
            )
        );
...
//            DeleteVinylRequest request = new DeleteVinylRequest(vinylVm.getVinyl().getTitle());
        ...
        Runnable onFinishedAction)
    {
        ...
        RotateTransition firstHalf = new RotateTransition(
            Duration.millis(150),
            card
        );
        ...
        RotateTransition secondHalf = new RotateTransition(
            Duration.millis(150),
            card
        );
